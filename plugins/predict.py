...
def get_days():
    ...
    today = pd.Timestamp(year=2015, month=today.month, day=today.day)
    monday = pd.Timestamp(year=2015, month=today.month, day=today.day)
    while monday.dayofweek != 0:
        monday += pd.Timedelta(days=1)
    if monday.year == 2016:
        monday = pd.Timestamp(year=2017, month=today.month, day=today.day)
        while monday.dayofweek != 0:
            monday -= pd.Timedelta(days=1)
    return today, monday
...
def predict_week(ti):
    ...
    print("Predicted ingredients for the next week:")
    print(weekly_ingredients)
